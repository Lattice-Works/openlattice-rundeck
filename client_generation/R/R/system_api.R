...
#' }
#'
#' \strong{ system_info_get } \emph{ Get Rundeck server information and stats }
#' 
#'
#' \itemize{
#' \item \emph{ @returnType } \link{SystemInfo} \cr
#'
#'
#' \item status code : 200 | System info response
#'
#' \item return type : SystemInfo 
#' \item response headers :
#'
#' \tabular{ll}{
#' }
...
SystemApi <- R6::R6Class(
  ...
  public = list(
    ...
    system_info_get = function(...){
      apiResponse <- self$system_info_getWithHttpInfo(...)
      resp <- apiResponse$response
      if (httr::status_code(resp) >= 200 && httr::status_code(resp) <= 299) {
        apiResponse$content
      } else if (httr::status_code(resp) >= 300 && httr::status_code(resp) <= 399) {
    ...
    system_info_getWithHttpInfo = function(...){
      args <- list(...)
      queryParams <- list()
      headerParams <- c()

      urlPath <- "/api/26/system/info"
      # API key authentication
      if ("X-Rundeck-Auth-Token" %in% names(self$apiClient$apiKeys) && nchar(self$apiClient$apiKeys["X-Rundeck-Auth-Token"]) > 0) {
        headerParams['X-Rundeck-Auth-Token'] <- paste(unlist(self$apiClient$apiKeys["X-Rundeck-Auth-Token"]), collapse='')
      }
